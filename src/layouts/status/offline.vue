<template>

  <div class="layout-offline inherit">

    <div>

      <v-svg width="90%" max-width="128px">
        <svg viewBox="0 0 24 24">
          <path fill="currentColor"
                d="M13,19H14A1,1 0 0,1 15,20H15.73L13,17.27V19M22,20V21.18L20.82,20H22M21,22.72L19.73,24L17.73,22H15A1,1 0 0,1 14,23H10A1,1 0 0,1 9,22H2V20H9A1,1 0 0,1 10,19H11V17H4A1,1 0 0,1 3,16V12A1,1 0 0,1 4,11H6.73L4.73,9H4A1,1 0 0,1 3,8V7.27L1,5.27L2.28,4L21,22.72M4,3H20A1,1 0 0,1 21,4V8A1,1 0 0,1 20,9H9.82L7,6.18V5H5.82L3.84,3C3.89,3 3.94,3 4,3M20,11A1,1 0 0,1 21,12V16A1,1 0 0,1 20,17H17.82L11.82,11H20M9,7H10V5H9V7M9,15H10V14.27L9,13.27V15M5,13V15H7V13H5Z"/>
        </svg>
      </v-svg>

      <h2>SERVICE UNREACHABLE</h2>

      <p>
        <small>Poked the Laravel service for some time but it did not respond...</small>
      </p>

      <p>
        <small>
          Are you connected to the internet?<br/>
          Maybe a service is down somewhere, try again later...
        </small>
      </p>

      <template v-if="isInitializing">
        <p>Retrying...</p>
      </template>

      <template v-else>
        <p>Automatic retry in {{ countdown }} seconds...</p>
      </template>

    </div>

  </div>

</template>

<script>
import { config } from '@/config'
import { mapActions } from 'vuex'

export default {
  name: 'layout-status-offline',

  data: () => ({
    interval: null,
    lastAttempt: null,
    countdown: null
  }),

  methods: {

    ...mapActions('Laravel', ['initialize']),

    start () {
      this.countdown = config.requestRetry
      this.lastAttempt = Date.now()
      this.interval = setInterval(this.retry, 999)
    },

    retry () {
      this.countdown = Math.round((this.lastAttempt - Date.now()) / 1000) + config.requestRetry

      if (this.countdown < 1) {
        clearInterval(this.interval)
        this.initialize()
      }
    }

  },

  mounted () {
    this.start()
  },

  beforeDestroy () {
    clearInterval(this.interval)
  }
}
</script>

<style lang="scss">

  .layout-offline {

    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 16px;
    color: white;
    background: rgb(202, 46, 63);
    background-image: url(/images/background.svg);
    background-image: url(/images/background.svg), linear-gradient(315deg, rgb(202, 46, 63) 0%, rgb(202, 70, 75) 100%);
    background-repeat: no-repeat;
    background-position: center center;
    background-size: contain;

    > div {

      display: inline-flex;
      width: 95vw;
      max-width: 480px;
      flex-direction: column;
      align-items: center;
      justify-content: space-between;

    }

    h2 {
      margin-top: 32px;
    }

    p {
      text-align: center;
    }

  }

</style>
